...
function ambilItem(inputNamaSheet,username,noKolom,idLokal,idTampil){
  var action="getItem";
...
function cekStatus(){
  ...
  if(status!='logged-in'){
    ...
    localStorage.setItem('usernameOnline',null);
...
function cekLoginOnline(){
  ...
  ambilItem(inputNamaSheet,username,1,'usernameOnline');
  ambilItem(inputNamaSheet,username,2,'nama');
  ambilItem(inputNamaSheet,username,3,'passwordOnline');
}
...
//if(usernameOnline!=null && passwordOnline!=null){
	...
	if(username!=usernameOnline || password!=passwordOnline){localStorage.setItem('status','Anda belum login (validasi)');}
	if(username==usernameOnline && password==passwordOnline){localStorage.setItem('status','logged-in');}
// }
}
function logOut(){
  localStorage.clear();
...
function cekLogoutHalLogin(){
	...
	document.getElementById('status_hal_login').innerHTML='Anda belum login'; 	  
	}
...
function simpanItem(idData){
  ...
  var action="postItem";
  username=localStorage.getItem('usernameOnline');
